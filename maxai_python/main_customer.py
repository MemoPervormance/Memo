"""
CroixAI — Customer Entry Point.
Requires a valid license key before backend selection.
"""
...
from license import (
    validate, check_cached, get_hwid, revoke_local,
    LicenseStatus,
)
...
def _license_gate() -> None:
    """Display banner and validate license before continuing. Exits on failure."""
    print(BANNER)
    print("=" * 60)
    print("  Hardware-ID:", get_hwid())
    print("=" * 60)

    # 1. Try cached license first (fast path, no network)
    status, msg = check_cached()
    if status == LicenseStatus.VALID:
        print(f"  ✓  Lizenz aktiv  (Cache)\n")
        logger.info(f"License: cached valid")
        return
    if status == LicenseStatus.OFFLINE_GRACE:
        print(f"  ⚠  {msg}\n")
        logger.info(f"License: offline grace period")
        return

    # 2. Need key entry
    if status != LicenseStatus.NO_KEY:
        print(f"  ✗  {msg}")

    print()
    print("  Bitte gib deinen Lizenz-Key ein.")
    print("  Format: CROIX-XXXX-XXXX-XXXX-XXXX")
    print("  [R] Key zurücksetzen  |  [Q] Beenden")
    print()
    ...
    while attempts < 5:
        ...
        print("  Validiere…", end="", flush=True)
        status, msg = validate(raw)
        print("\r" + " " * 20 + "\r", end="")  # clear "Validiere…"

        if status == LicenseStatus.VALID:
            print(f"  ✓  {msg}\n")
            logger.info(f"License: validated online — key={raw[:8]}…")
            return

        if status == LicenseStatus.OFFLINE_GRACE:
            print(f"  ⚠  {msg}\n")
            logger.info(f"License: offline grace")
            return

        # Failure
        attempts += 1
        left = 5 - attempts
        print(f"  ✗  {msg}  ({left} Versuch{'e' if left != 1 else ''} verbleibend)\n")
        logger.warning(f"License rejected: status={status} msg={msg}")
...
def _parse_args():
    p = argparse.ArgumentParser(description="CroixAI Target Tracker — Licensed Build")
    ...
    p.add_argument("--skip-license", action="store_true",
                   help=argparse.SUPPRESS)   # hidden dev flag
    return p.parse_args()
